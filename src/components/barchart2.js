...
async function getHistoricalRates(apiKey,startDate,endData,symbol,base)
{
  ...
  try {
    ...
    for(let key in data.rates)
      myData.push(data.rates[key].USD);
    for(let key in data.rates)
...
getHistoricalRates(getApiCode(),getStartDate(),getEndDate(),getSymbol("USD"),getBase("GBP"));

  const chartData = {
    labels: myDays|| [], // Use provided labels or empty array
    datasets: [
      {
        label: 'USD/GBP BarChart',
        data: myData || [],
...
export default React.memo(BarComponent2);
