...
async function getHistoricalRates(apiKey,startDate,endData,symbol,base)
{
  ...
  try {
    ...
    for(let key in data.rates)
      myData.push(data.rates[key].EGP);
    for(let key in data.rates)
...
getHistoricalRates(getApiCode(),getStartDate(),getEndDate(),getSymbol("EGP"),getBase("USD"));

  const chartData = {
    labels: myDays|| [], // Use provided labels or empty array
    datasets: [
      {
        label: 'USD/EGP BarChart',
        data: myData || [],
...
export default React.memo(BarComponent1);